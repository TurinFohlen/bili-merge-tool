...
class ExtractorDash:
    ...
    def extract(self, uid: str, c_folder: str, quality: str, temp_dir: str, fmt: str) -> tuple:
        ...
        # 音频文件：在视频所在目录直接查找（避免二次递归可能的问题）
        video_dir = os.path.dirname(video_src)
        a_names = ["audio.m4s", "audio.mp4", "audio.m4a", "audio.mp3"] if fmt == "dash" else ["audio.mp4", "audio.m4s", "audio.m4a", "audio.mp3"]
        print(f"  🔍 在视频目录查找音频文件: {a_names}")
        audio_src = None
